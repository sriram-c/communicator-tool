...
class complex_sent_handler():
    ...
    for row in x:
        headerdict[row.split('\t')[0]] = row.split('\t')[1].strip('\n') # xheaderdict has left column as key and right column as value:
...
####################################################/
    ...
    for word in relclauselist:   
        if word == ',jo,' and word in headerdict['s']:
            if 'jo_1' in headerdict['c']:
                
                rules = open(directory+'/../../rule_templates/rel.txt','r')
                ruletxt = rules.readlines()
                
                pronoun_eng = open(CommunicatorPath+'/dic/pronoun_lemma_dic.txt',"r")
                
                global prontxt
                prontxt = pronoun_eng.readlines()
                
                for line in prontxt:
                    if 'jo_1,' in line:
                        global wh_word
                        wh_word = line.split(',')[2].split('	')[1]
                #print prontxt
    ...
    #print headerdict
                headerdict["s2.g''"] = headerdict["s2.g''"].replace('wh',wh_word)
