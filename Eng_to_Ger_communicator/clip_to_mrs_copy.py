...
def named_(li,xi): #To split CLIPS fact and store information of named(proper noun) nodes
	file2.write(" ]\n [ ")
        word =li.split()
        str1=word[1]+'<-1:-1>'+" LBL: "+word[3]+" CARG: "+word[4].replace(')','')+" ARG0: "+word[2]
        file2.write(str1)
	del nameddic[xi]
...
def name_(li,xi):  #To split CLIPS fact and store information of all nodes
	...
	if xi in rstdic.keys():
		if '_q' in word[1]:
			rs_(rstdic[xi],xi)
	if xi in mrsdic.keys():
		mrs_(mrsdic[xi],xi)
	if xi in nameddic.keys():
...
#		#print global lb,global ar
	...
	if (line.find('topic-or-focus_d_rel')!=-1):
		line5=line
		word=line5.split()
		str1="\n ["+word[0].replace('(',' ')+""+word[1]+" "+word[2]+" "+word[3]+" "+word[4]+" "+word[5]+" "+word[6]+" "+word[7].replace(')','')+" ] >\n"
	#	str1="\n [ "+line+" ] >\n"
	file2.write(str1)
	new_str=new_str+'> ]'		
	file2.write(new_str)
